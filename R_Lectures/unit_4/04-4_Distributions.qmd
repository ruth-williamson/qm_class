...
flips |> ggplot(aes(x = Flips, y = Ways)) + geom_bar(stat = "Identity")
```
...
```{r}
#| fig-alt: "Histogram showing the relative probabilities of different combinations of heads and tails for 6 flips."

flip6 <- tibble(
  Ways = c(1, 6, 15, 20, 15, 6, 1),
  Flips = c("0 H", "1 H", "2 H", "3 H", "4 H", "5 H", "6 H"),
  Prob = Ways / sum(Ways)
) |>
  mutate(Flips = fct_inorder(Flips))
...
```{r}
#| message: false
#| fig-alt: "Histogram of 200 random numbers."
...
```{r}
#| message: false
#| fig-alt: "Histogram of 200 random numbers with a vertical red line at 1.7."
...
#| fig-alt: "Density plot of 200 random numbers."
...
```{r}
#| message: false
#| fig-alt: "Density plot of 200 random numbers overlying a histogram of the same observations."
...
```{r}
#| fig-alt: "Density plot of 200 random numbers showing a nearly perfect normal distribution."
#|
M <- tibble(
...
ggplot(M, aes(x, y)) + geom_line() + labs(y = "Probability Density")
```
...
```{r}
#| fig-alt: "Histogram of binomial probabilities from 0 to 10 with a probability of 0.8."

M <- tibble(Successes = 0:10, Probability = dbinom(0:10, 10, prob = 0.8))
...
```{r}
#| fig-alt: "Histogram of Poisson probabilities from 0 to 20 with a mean of 5."

trials <- 20

M <- tibble(Successes = 0:trials, Probability = dpois(0:trials, lambda = 5))
ggplot(M, aes(Successes, Probability)) +
  geom_bar(stat = "identity") +
...
![](https://i.imgur.com/Mks8NPD.png"){fig-align="center" width="80%" fig-alt="Figure showing a range of different distributions and their interrelationships."}
