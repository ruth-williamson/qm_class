...
<!--
Datasets
  soil_respiration.csv
  EVI.csv
  rattle_probability.csv
  rattle_duration.csv
  rattle_rate.csv
  gallfly_mating.xlsx
-->
...
For this part of the progress check you will be given a dataset, and a series of questions to answer about the data, or tasks to perform with the data. Use what you learned about how to create clean, well-commented code to answer each of the questions in Markdown format so you could return to this code in 6 months and understand what you did.
...
![](https://imgur.com/CwCrXha.png){width=50% fig-align="center"}
...
```{r}
soil_resp_og <- read_csv("soil_respiration.csv") #reading in soil respiration data using read_csv 

#data transformed from wide to long format:
soil_resp <- soil_resp_og |>
  pivot_longer(cols = 2:24 , names_to = "plot", values_to = "Rs")
#cols = 2:24 are the columns that need to be pivoted, the numbering system is not ideal but I am unsure how to call the columns without renaming them
#names to names the new column made from the names of the previous columns
#values to names the new column made from the values of the table
...
```{r}
#grouped by plot and crop type
soil_resp |>
  ggplot(aes(x=Crop, y=Rs, color = factor(plot))) + #plotting with crop on the x axis and Rs on the y colored by plot
  geom_point(position = position_jitter(width = 0.3), #position jitter moved the dots so they aren't on top of each other
    alpha = 0.5, #alpha makes the dots slightly transparent
    size = 2) + #size control of dots
  geom_boxplot(alpha = 2 / 3, outlier.shape = NA) + #making the boxplots slightly transparent and turning off the outliers they don't get confused with the actual data points
  labs(x= "Crop Type", y= "Soil Respiration", color = "Plot #")

```

```{r}
#grouped by just crop type
soil_resp |>
  ggplot(aes(x=Crop, y=Rs, color = Crop)) + #plotting with crop on the x axis and Rs on the y colored by plot
  geom_point(position = position_jitter(width = 0.3), #position jitter moved the dots so they aren't on top of each other
    alpha = 0.5, #alpha makes the dots slightly transparent
    size = 2) + #size control of dots
  geom_boxplot(alpha = 2 / 3, outlier.shape = NA) + #making the boxplots slightly transparent and turning off the outliers they don't get confused with the actual data points
  labs(x= "Crop Type", y= "Soil Respiration", color = "Crop Type") #editing labels
```

Reflect on the data. Does everything seem okay? Are outliers present?

> Yes, one of the data points is around 40 and the rest are lower than 10
...
```{r}
#changing the outlier value
soil_resp_cor <- soil_resp |>
  mutate(Rs = replace(Rs, Rs==40.04, 4.04)) #replaces this specific error with the correction

#removing plot number 12 entirely because of the incorrect data
soil_resp_cor2 <- soil_resp |>
  mutate(Rs = replace(Rs, Rs == 40.04, NA))

#grouped by just crop type now with corrected 4.04
soil_resp_cor |>
  ggplot(aes(x=Crop, y=Rs, color = Crop)) + #plotting with crop on the x axis and Rs on the y colored by plot
  geom_point(position = position_jitter(width = 0.3), #position jitter moved the dots so they aren't on top of each other
    alpha = 0.5, #alpha makes the dots slightly transparent
...
```{r}
#plotting a density plot of the two crop types and their soil respiration
soil_resp_cor |>
  ggplot() +
  geom_density(aes(x = Rs , fill = Crop), alpha=0.5) + #tells the density plot what to plot and fill is what to group by, alpha makes them slightly transparent so you can see both
  labs(x= "Soil Respiration", y= "Density", color = "Crop Type")

```

What can you conclude about the distributions of the data?

> The summer maize has a much higer distribution of values near 4.8 and 5 while the winter wheat generally has a higher distribution near the values of 3, but even then it is more evenly distributed across the values.

Summarize this data by calculating a mean and standard deviation for soil respiration for each crop type.

```{r}
#making a summary table of the mean and sd of the two crop type's respiration values
soil_m_sd <- soil_resp_cor |>
  group_by(Crop) |>
  summarize(mean(Rs), sd(Rs))
...
Using your long dataset that has the 40.04 value replaced with 4.04, merge the EVI dataset (`EVI.csv`) with the soil respiration data. You will notice that the EVI dataset does not currently have unique indicators (or key values) to merge with the soil respiration data.  You can do two things, either 1) join based on multiple columns (save as `new1`), or 2) create a unique identifier column for both datasets (using for example, `mutate()` and `paste()` to combine the similar columns) and join with the unique ID column (save as `new2`). Practice both below. (NOTE: This example and the extra data wrangling you need to do here shows why you should always have plot numbers or some other unique value for each of your datasets.)

```{r}
evi_og <- read_csv("EVI.csv") #reading in soil respiration data using read_csv from tidyverse

#joining by merging using multiple columns
new1 <- left_join(soil_resp_cor, evi_og, by= join_by(Crop, Rs))

#joining by adding identifying column
soil_resp_cor_id <- soil_resp_cor |>
  mutate(ID = paste(Crop, Rs, sep= "_"))

evi_id <- evi_og |>
  mutate(ID = paste(Crop, Rs, sep = "_"))
# think a flaw in this would occur if there were any duplicate readings of the crop Rs values

new2 <- left_join(soil_resp_cor_id, evi_id, by= join_by(ID))
```
...
```{r}
#relationship between EVI and RS
new1|>
...
new1 |>
  ggplot(aes(x=Crop, y=EVI, color = Crop)) + #plotting with crop on the x axis and Rs on the y colored by plot
  geom_point(position = position_jitter(width = 0.3), #position jitter moved the dots so they aren't on top of each other
    alpha = 0.5, #alpha makes the dots slightly transparent
    size = 2) + #size control of dots
  geom_boxplot(alpha = 2 / 3, outlier.shape = NA) + #making the boxplots slightly transparent and turning off the outliers they don't get confused with the actual data points
  labs(x= "Crop Type", y= "EVI", color = "Crop Type") 
...
rattle_prob <- read_csv("rattle_probability.csv")
rattle_dur <- read_csv("rattle_duration.csv")
rattle_rate <- read_csv("rattle_rate.csv")
```
...
```{r}
#renaming the columns to be more managable
rattle_dur <- rattle_dur |>
  rename(species = Species, distance = `Species'DistanceToRattlesnakeNode`, duration = `Species'MeanDurationOfVibration`)

rattle_prob <- rattle_prob |>
  rename(species = Species, distance = `Species'DistanceToRattlesnakeNode`, probability = `PercentOfSpeciesVibrating`)

rattle_rate <- rattle_rate |>
  rename(species = Species, distance = `Species'DistanceToRattlesnakeNode`, rate = `Species'MeanRateOfVibration`)
...
```{r}
nrow(rattle_dur) #counting the number of rows
rowstokeep <- seq(2, 82, by=2) #making a list of numbers counting every other starting at 2 and going until 82

rattle_dur <- rattle_dur |>
  slice(rowstokeep) #slicing and keeping only the even rows which have the data
```

```{r}
nrow(rattle_prob) #counting the number of rows
rowstokeep <- seq(2, 110, by=2) #making a list of numbers counting every other starting at 2 and going until 110
...
```{r}
nrow(rattle_rate) #counting the number of rows
rowstokeep <- seq(2, 82, by=2) #making a list of numbers counting every other starting at 2 and going until 110
...
```{r}
og_rattle_dur <- read_csv("rattle_duration.csv")
is.na(rattle_dur)
is.na(og_rattle_dur)
#checks to make sure our new file doesn't have na and our old one did.
```
...
```{r}
combine_step1 <- left_join(rattle_prob, rattle_dur, by = join_by(species, distance))
rattle_comb <- left_join(combine_step1, rattle_rate, by = join_by(species, distance))
```
...
summarize(rattle_comb, prob_na = sum(is.na(rattle_comb$probability)), dur_na = sum(is.na(rattle_comb$duration)), rate_na = sum(is.na(rattle_comb$rate)))

#this creates a summary table that shows how many na's are in each row. 
# rate_na is the column of the summary table and then = sum(is.na(the data frame and the column you want to summarize))
```

How many `NA`s does each column have? Can you explain the pattern of missing data?

> Duration and Rate both have 14 missing data each and probability has 0. This means that probability has 14 additonal species of snakes than the other two.
...
```{r}
rattle_long <- rattle_comb |>
  pivot_longer(cols = c(probability,duration,rate), names_to = "measurement", values_to = "value")
```
...
```{r}
rattle_long|>
  ggplot(aes(x=distance, y=value, color = measurement )) +
  geom_point(size = 3, alpha = 0.5) +
  facet_grid(measurement~., scales = "free_y")+ 
  labs(x = "Distance from Node", y = "Measurement Values", color = "Rattle: ")
```
...
> as you move farther from the node, there is a negative relationship to the variables measured. The less related they are the lower probability for high rattle duration, probability, and rate.

Return to the "wide" tibble. Now plot Rate as a function of Duration, color the points by Distance, and add a linear fit.

```{r}
rattle_wide <- rattle_long |>
  pivot_wider(names_from = measurement, values_from = value)

rattle_wide|>
  ggplot(aes(x=duration, y=rate, color = distance )) +
  geom_point(size = 3, alpha = 0.5) +
  geom_smooth(formula = y~x, method = "lm", se=FALSE) +
  labs(x = "Rattle Duration", y = "Rattle Rate", color = "Distance from Node")
```
...
> There does not seem to be a any association with distance from node and rattle rate and duration. You can see that there is a correlation between longer durations having higher rates but the distance is all over the place. You would expect all the blue ones to be close to the bottom left corner and the blue ones to be more near the top right corner.
...
> Yes, because you can't be two species at a time.
...
> I would suggest grouping them by distance and then doing analysis since there main question was "how does distance from the node effect rattle traits"
...
```{r}
gfly_mating <- read_excel("gallfly_mating.xlsx", na="NA") #importing data
```

```{r}
#cleaning data file
glimpse(gfly_mating) #plot is not all in numeric form as we can see from the data type being a character
gfly_mating |>
  count(host_race) #Alt and Gig
gfly_mating |>
  count(plant_choice) #some are ALT, Alt, Gig, Gigantia, and NA

gfly_mating_cleaned <- gfly_mating |>
  mutate(host_race = tolower(host_race), 
         plant_choice = tolower(plant_choice), 
         plant_choice = str_replace(plant_choice, "gigantia", "gig"), 
         plot = str_replace(plot, "plot 1", "1"), 
         plot = str_replace(plot, "plot 2", "2"), 
         plot = str_replace(plot, "plot 3", "3")) # got rid of the different case and changed gigantia to gig then changed the plot labels to the correct values.
...
For the pairwise combinations of `host_race` and `plant_choice`, tally up the number of ovipositions for each. Assign this tibble to `gall_counts`. You can print a nice copy of this table to your qmd output with the code `knitr::kable(gall_counts)`. Add this code to your chunk after your tallying code.
...
The fifth week is a good place to reflect a bit on your learning progress so far in this class. Consider where you were at the start and where you are now.

What are 3 skills you have learned?

> 


What are 3 areas that you want to improve between now and the next progress check?

> 


What are 3 things that helped you learn?

>


What are 3 things you will remember for a long time?
