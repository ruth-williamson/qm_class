...
```{r}
sd <- sd(1:6) #naming standard deviation of vector 1-6 as "sd"
sd #called the object "sd" to print
sd(4:10) #used the standard deviation function to get the standard deviation of 4-10
```
...
> The first line is assigning the name of the vector object "sd" to the standard deviation. Then we called the standard deviation of our object named sd. The final line is the standard deviation of 4 through 10.
...
```{r}
num_vect <- c(1, 6, 10, 14.75)
logic_vector <- c(TRUE, TRUE, FALSE)
char_vector <- c("a", "aa", "aaa")
by_one <- seq(5,10)
by_five <- seq(5,100, by=5)
sixty_val <- seq(5,100, length.out=60)
sv_ten <- rep(17, times=10)
sq_one <- rep(c(1,2,3), each=10)
sq_two <- rep(c(1,2,3), times=10)
```
...
```{r}
a <- sqrt(2) # storing square root of 2 as "a"
...
> line 1 stores square root of 2 as "a". 
Line two prints "a". 
Line 3 multiply "a" by itself to get 2. 
Line 4 check if "a" * "a" equals 2 and it doesn't, so they are stored as with different decimals so they are not perfectly equal.
Line 5 checks if they are almost equal and they are indeed almost equal due to rounding and fits due to the margin of acceptible error. 
...
```{r}
data1 <- matrix(rnorm(36, mean=10, sd=5), nrow=6, ncol= 6)
data1
```
...
> logical matrix is returned

(d) Use your inequality from part (c) and `colMeans()` to calculate the proportion of values less than 10 in your matrix.

```{r}
colMeans(data1 <10) # gives proportion of 1s and 0s to find the proportion of values less 10
```
...
>R is making true into 1 and false into 0 so when you ask for the mean it gives you the proportion of trues and falses in the columns
...
our_tibble <- tibble(
  ...
  Replicate = rep(c(1,2), times = 10),
  Productivity = c(rnorm(10, mean= 5, sd = 2), rnorm(10,mean=8, sd= 2))
)
```
...
median(x)
t(x)
x^2 #is applied individually rather than the whole data set

```

```{r}
mean(our_tibble$Productivity)
sd(our_tibble$Productivity)
median(our_tibble$Productivity)
t(our_tibble$Productivity)
```
...
```{r}
#create data frame with columns x,y,l. Where x counts up from 1 to 20 and then y counts down from 20 to 1. 
M1 <- data.frame(
  x = 11:20, 
  y = 20:11, 
  l = letters[1:10]
...
#create a tibble from 3 columns. The ID column is from 1 to 100. The x column is a normal distribution with a mean of 100 and a sd of 5.
M2 <- tibble(
...
> uses the letters function to fill the l column with a through j, in lowercase.

Explain how `y` is constructed in `M2`.

> y is constructed by randominlgy generating numbers with a mean of 0 and a sd of 1 which is added to x and then multiplied by a constant value 8.4. 
...
> I did not use AI in this problem set. 
