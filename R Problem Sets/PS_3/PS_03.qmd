...
```{r}
#none of the people are left handed 
dbinom(0, 24, prob = 0.1)

#one person is left handed
dbinom(1, 24, prob = 0.1)

#one or more are left handed
sum(dbinom(1:24, 24, prob = 0.1))
```

```{r}
#binomial coefficient
#these successes could happen in any number, three things, choose two
choose(3, 2)

choose(69, 5) * choose(26, 1) #chance of getting when 

2* choose(69, 5) * choose(26, 1) #expected value when the cost of ticket would give more than 2 dollars the price of the ticket
```
...
richness3 <- comp |>
  ...
  summarize(richness = length(unique(species))) #copying from answer key now
```
...
```{r}
richness <- left_join(richness,plot_ids)
...
```{r}
richness_sp |>
  ggplot(aes(x=rain_removal, y=richness, color=factor(plot_no))) +
  geom_point(size = 3,
