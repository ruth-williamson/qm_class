...
> missing data points with no NA.
> each observation had it's own column rather than it's own row.
...
```{r}
gr_long <- gr |> pivot_longer( starts_with("day"),
                               names_to = "Day",
                               values_to = "Biomass")
...
> Now this is tidy because it has one variable per column and one observation per row and each value has it's own cell with NA being missing values.
...
> Yes, we have 55 in each species group. 
...
> we do still have several NA's in our data set and it is reading the code as if it is a real number thus there are equal sample sizes shown in the previous step, however this shouldn't be.
...
> No, some of them have missing data points so the groups are no longer equal.
...
```{r}
gr_long2 <- gr_long |> drop_na()

gr_long2 |> group_by(species) |>
  tally()
...
```{r}
gr_long2 |> ggplot(
  aes(x= days, y=Biomass, color = species)) + 
  geom_point() +
  geom_jitter() +
  geom_smooth(se=F)
...
> Yes, it helps spread apart the difference in the data and makes it easier to see the relative differences between the groups.
